...
create table item (text_parsed boolean not null, context_id bigint not null, date_archived timestamp(6), date_created timestamp(6), date_expire timestamp(6), date_updated timestamp(6), file_id bigint unique, issue_date timestamp(6), item_id bigint not null, store_id bigint, user_id bigint not null, version bigint, uuid varchar(16) not null unique, source varchar(20) check (source in ('UPLOAD','EMAIL','SCAN','API','UNKNOWN')), attributes json, description varchar(255), icon varchar(255), name varchar(255) not null, stage varchar(255) check (stage in ('ACTIVE','ARCHIVED','DELETED')), storage_item_identifier varchar(255), semantic_embedding float4 array, constraint UNIQUE_ID primary key (item_id));
create table item_category (category_id bigint not null, item_id bigint not null);
create table item_metadata (file_id bigint not null, filesize bigint not null, crypt_settings varchar(255) not null, filename varchar(255) not null, mimetype varchar(255) not null, sha256crypted varchar(255) not null unique, sha256original varchar(255) not null unique, source varchar(255) not null, primary key (file_id));
...
create index attributetype_uuid_index on attributetype (uuid);
create index IDX_CATEGORY_UUID on category (uuid);
create index IDX_CATEGORY_PARENT_NAME on category (parent_category_id, name);
create index index_config_key on config (config_key);
create index context_uuid_index on context (uuid);
create index user_uuid_index on dbuser (uuid);
create index item_uuid_index on item (uuid);
create index item_issueDate_index on item (issue_date);
create index item_source_index on item (source);
create index item_context_index on item (context_id);
create index store_uuid_index on store (uuid);
