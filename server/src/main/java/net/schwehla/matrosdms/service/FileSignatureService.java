 ...
 * This avoids the overhead of loading the entire Tika framework for common file
 * types.
 *
...
public class FileSignatureService {
	...
	public String quickDetect(Path file) {
		try (InputStream is = Files.newInputStream(file)) {
			...
			if (startsWith(header, SIG_PDF))
				return "application/pdf";
			if (startsWith(header, SIG_PNG))
				return "image/png";
			if (startsWith(header, SIG_JPG))
				return "image/jpeg";
