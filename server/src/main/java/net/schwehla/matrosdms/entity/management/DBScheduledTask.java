...
@NamedQueries({
        ...
        @NamedQuery(name = "DBScheduledTask.findByNameAndInstance",
                query = "SELECT t FROM DBScheduledTask t WHERE t.id.taskName = :taskName AND t.id.taskInstance = :taskInstance"),
        @NamedQuery(name = "DBScheduledTask.findDue",
                query = "SELECT t FROM DBScheduledTask t WHERE t.executionTime <= :now AND t.picked = false ORDER BY t.priority DESC, t.executionTime ASC")
})
