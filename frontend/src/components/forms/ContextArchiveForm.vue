...
const onConfirm = async () => {
  const promise = push.promise('Closing context...')
  try {
    const { error, response } = await client.DELETE('/api/contexts/{id}', {
      params: { path: { id: dms.selectedContext.uuid } }
    })
    
    // Handle 409 Conflict - context has items
    if (response?.status === 409) {
      promise.reject('Cannot delete folder. Please move or delete items inside it first.')
      return
    }
    ...
    dms.cancelCreation()
    promise.resolve('Context closed')
  } catch(err) {
...
<template>
  <div class="h-full flex flex-col bg-muted/20 transition-colors">
    <div class="p-2 border-b border-border bg-background h-[35px] flex items-center shadow-sm">
      <span class="text-[13px] font-bold text-destructive flex items-center gap-2"><Archive :size="14"/> Close Context</span>
    </div>
    <div class="flex-1 p-6 flex flex-col items-center justify-center text-center overflow-auto">
      <div class="bg-background p-8 rounded-lg border border-border shadow-sm max-w-md w-full transition-colors">
        ...
        <h3 class="text-lg font-bold text-foreground mb-2">Close "{{ dms.selectedContext?.name }}"?</h3>
        <p class="text-sm text-muted-foreground mb-6">
          This context will be marked as closed. It will no longer appear in the active list unless you change your filter settings.
        </p>
